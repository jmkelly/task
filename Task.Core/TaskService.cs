...
namespace Task.Core
{
	public class TaskService : ITaskService
	{
		...
		public TaskService(DatabaseConnectionSettings settings)
		{
			...
			CancellationToken cancellationToken = default)
		{
			...
			if (!string.IsNullOrEmpty(sortBy))
			{
				...
				{
					"title" => sortOrder?.ToLowerInvariant() == "desc" ? tasks.OrderByDescending(t => t.Title).ToList() : tasks.OrderBy(t => t.Title).ToList(),
					"priority" => sortOrder?.ToLowerInvariant() == "desc" ? tasks.OrderByDescending(t => t.Priority).ToList() : tasks.OrderBy(t => t.Priority).ToList(),
					"due_date" => sortOrder?.ToLowerInvariant() == "desc" ? tasks.OrderByDescending(t => t.DueDate).ToList() : tasks.OrderBy(t => t.DueDate).ToList(),
					"created_at" => sortOrder?.ToLowerInvariant() == "desc" ? tasks.OrderByDescending(t => t.CreatedAt).ToList() : tasks.OrderBy(t => t.CreatedAt).ToList(),
					"updated_at" => sortOrder?.ToLowerInvariant() == "desc" ? tasks.OrderByDescending(t => t.UpdatedAt).ToList() : tasks.OrderBy(t => t.UpdatedAt).ToList(),
					_ => tasks
				};
			}
