...
namespace Task.Cli
{
	public class ConfigSetCommand : AsyncCommand<ConfigSetCommand.Settings>
	{
		...
		public class Settings : CommandSettings
		{
			...
			[Description($"The configuration key to set ({SupportedKeys})")]
	...
	public class ConfigGetCommand : AsyncCommand<ConfigGetCommand.Settings>
	{
		...
		{
			[CommandArgument(0, "<key>")]
			[Description($"The configuration key to get ({SupportedKeys})")]
			public string Key { get; set; } = null!;
		}
	...
	public class ConfigUnsetCommand : AsyncCommand<ConfigUnsetCommand.Settings>
	{
		...
		{
			[CommandArgument(0, "<key>")]
			[Description($"The configuration key to unset ({SupportedKeys})")]
			public string Key { get; set; } = null!;
		}
	...
	public class ConfigListCommand : Command<ConfigListCommand.Settings>
	{
		...
		public override int Execute(CommandContext context, Settings settings, CancellationToken cancellationToken)
		{
			...
			Console.WriteLine($"database.pg.connectionString: {config.Database?.Postgres?.ConnectionString ?? "(not set)"}");
