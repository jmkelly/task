...
2. **Task.Cli** (Renamed from current "Task" project): Pure CLI application with local DB support.
3. **Task.Api** (Existing project): Standalone web API with Database 

## Detailed Implementation Plan

### Phase 1: Prepare Shared Components (Task.Core)
- Create a new .NET class library project for shared code.
- Move unified `TaskItem` model (resolve CLI/API differences: standardize status defaults to "todo"/"in_progress"/"done").
...
- Modify tests to focus on CLI-specific functionality.
...
- Run full test suites in all projects.
- Validate CLI works correctly in API integration mode.
...
3. Ensure status defaults ("todo"/"in_progress"/"done") are consistent between CLI and API.
4. Extensions will be kept local to their respective projects. Binaries will be in their own directory (shared reference) as it current is.

## Success Criteria
- Clean separation with between CLI and API repos.
- Both components build and run independently.
- CLI only supports API integration mode.
