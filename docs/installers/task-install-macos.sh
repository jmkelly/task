...
resolve_binary_path() {
    ...
    fail "extract" "Downloaded asset did not contain a macOS Task CLI binary."
}
...
main() {
    ...
    if [ "$(uname -s)" != "Darwin" ]; then
        fail "platform" "This installer only supports macOS."
    fi
    ...
    case "$machine_architecture" in
        ...
        x86_64)
            ...
            ;;
        *)
            fail "platform" "macOS installer supports Apple Silicon and Intel release assets only. Detected architecture: ${machine_architecture}."
            ;;
    esac
